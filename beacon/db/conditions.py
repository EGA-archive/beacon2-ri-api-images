...
def get_conditions(entry_id: Optional[str], qparams: RequestParams):
    collection = 'conditions'
    query = apply_request_parameters({}, qparams)
    query = apply_filters(query, qparams.query.filters, collection)
    query = include_resultset_responses(query, qparams)
    schema = DefaultSchemas.CONDITIONS
    count = get_count(client.beacon.conditions, query)
    include = qparams.query.include_resultset_responses
    if include == 'MISS':
        pre_docs = get_documents(
            client.beacon.conditions,
            query,
        ...
        count = get_count(client.beacon.conditions, negative_query)
    else:
        docs = get_documents(
            client.beacon.conditions,
            query,
...
def get_variants_of_occurrence(entry_id: Optional[str], qparams: RequestParams):
    ...
    query = apply_filters(query, qparams.query.filters, collection)
    count = get_count(client.beacon.conditions, query)
    occurrence_ids = client.beacon.conditions \
