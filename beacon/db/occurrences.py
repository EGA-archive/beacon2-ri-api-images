...
def get_occurrences(entry_id: Optional[str], qparams: RequestParams):
    collection = 'occurrences'
    query = apply_request_parameters({}, qparams)
    query = apply_filters(query, qparams.query.filters, collection)
    query = include_resultset_responses(query, qparams)
    schema = DefaultSchemas.OCCURRENCES
    count = get_count(client.beacon.occurrences, query)
    include = qparams.query.include_resultset_responses
    if include == 'MISS':
        pre_docs = get_documents(
            client.beacon.occurrences,
            query,
        ...
        count = get_count(client.beacon.occurrences, negative_query)
    else:
        docs = get_documents(
            client.beacon.occurrences,
            query,
...
def get_occurrence_with_id(entry_id: Optional[str], qparams: RequestParams):
    collection = 'occurrences'
    query = apply_request_parameters({}, qparams)
    query = apply_filters(query, qparams.query.filters, collection)
...
def get_variants_of_occurrence(entry_id: Optional[str], qparams: RequestParams):
    collection = 'occurrences'
    query = {"$and": [{"id": entry_id}]}
    query = apply_request_parameters(query, qparams)
    query = apply_filters(query, qparams.query.filters, collection)
    count = get_count(client.beacon.occurrences, query)
    occurrence_ids = client.beacon.occurrences \
        .find_one(query, {"biosampleId": 1, "_id": 0})
    occurrence_ids=get_cross_query(occurrence_ids,'biosampleId','caseLevelData.biosampleId')
    query = apply_filters(occurrence_ids, qparams.query.filters, collection)
    query = include_resultset_responses(query, qparams)
...
def get_filtering_terms_of_occurrence(entry_id: Optional[str], qparams: RequestParams):
    query = {'scope': 'occurrences'}
