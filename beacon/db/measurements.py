...
def get_measurements(entry_id: Optional[str], qparams: RequestParams):
    collection = 'measurements'
    query = apply_request_parameters({}, qparams)
    query = apply_filters(query, qparams.query.filters, collection)
    query = include_resultset_responses(query, qparams)
    schema = DefaultSchemas.MEASUREMENTS
    count = get_count(client.beacon.measurements, query)
    include = qparams.query.include_resultset_responses
    if include == 'MISS':
        pre_docs = get_documents(
            client.beacon.measurements,
            query,
        ...
        count = get_count(client.beacon.measurements, negative_query)
    else:
        docs = get_documents(
            client.beacon.measurements,
            query,
...
def get_variants_of_occurrence(entry_id: Optional[str], qparams: RequestParams):
    ...
    query = apply_filters(query, qparams.query.filters, collection)
    count = get_count(client.beacon.measurements, query)
    occurrence_ids = client.beacon.measurements \
